...
@member_required
def redeem_hadiah_page(request):
    email = _current_email(request)
    member = services.get_member(email)
    context = {
        "member": member,
        "member_name": services.full_name(member),
        "award_miles": member["award_miles"] if member else 0,
        "katalog": services.get_katalog_hadiah(),
        "riwayat": services.get_riwayat_redeem(email),
    }
...
def process_redeem(request):
    ...
    if not kode_hadiah:
        ...
        return redirect("main:redeem_hadiah_page")

    try:
        services.execute_redeem(_current_email(request), kode_hadiah)
